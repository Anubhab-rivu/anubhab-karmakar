...
import { buildDiplomaDataSearchItems } from '@/src/data/diploma';
...
const diplomaExtras = [
  ...buildDiplomaSearchExtras(syllabusData),
  ...buildDiplomaDataSearchItems(),
];
...
function parseAdvancedQuery(raw) {
  ...
  const cleaned = raw
    .split(/\s+/)
    .filter((part) => {
      const match = part.match(/^(degree|type|sem|semester|section|unit):(.+)$/i);
      if (!match) return true;

      const key = match[1].toLowerCase();
      const value = match[2].toLowerCase();
      if (key === 'degree' && ['diploma', 'btech'].includes(value)) filters.degree = value;
      if (key === 'type' && ['subject', 'unit'].includes(value)) filters.type = value;
      if ((key === 'sem' || key === 'semester') && /^\d+$/.test(value)) filters.semester = `sem-${value}`;
      if ((key === 'sem' || key === 'semester') && /^sem-\d+$/.test(value)) filters.semester = value;
      if (key === 'section') filters.section = value;
      if (key === 'unit' && /^\d+$/.test(value)) filters.unit = Number(value);
      return false;
    })
    .join(' ')
    .trim();

  return { cleanQuery: cleaned, filters };
}
...
export function searchNotes(query, options = {}) {
  const parsed = parseAdvancedQuery(normalize(query).trim());
  const raw = parsed.cleanQuery;
  const hasInlineFilters = Object.keys(parsed.filters).length > 0;
  if (!raw && !hasInlineFilters) return [];

  const { phrases, tokens } = tokenize(raw);
  const mergedOptions = { ...options, ...parsed.filters };
  const degreeFilter = mergedOptions.degree && mergedOptions.degree !== 'all' ? mergedOptions.degree : null;
  const typeFilter = mergedOptions.type && mergedOptions.type !== 'all' ? mergedOptions.type : null;
  const semesterFilter = mergedOptions.semester && mergedOptions.semester !== 'all' ? mergedOptions.semester : null;
  const sectionFilter = mergedOptions.section && mergedOptions.section !== 'all' ? normalize(mergedOptions.section) : null;
  const unitFilter = mergedOptions.unit || null;

  return searchIndex
    .filter((item) => !degreeFilter || item.degree === degreeFilter)
    .filter((item) => !typeFilter || item.type === typeFilter)
    .filter((item) => !semesterFilter || item.semester === semesterFilter)
    .filter((item) => !unitFilter || item.title.toLowerCase().startsWith(`unit ${unitFilter}:`))
    .filter((item) => {
    ...
    .map((item) => {
      ...
      let score = hasInlineFilters && !raw ? 12 : 0;

      if (raw && title === raw) score += 80;
      if (raw && title.includes(raw)) score += 45;
      if (raw && subtitle.includes(raw)) score += 22;
      if (raw && tags.includes(raw)) score += 16;
      ...
      if (unitFilter && item.title.toLowerCase().startsWith(`unit ${unitFilter}:`)) score += 20;
      if (semesterFilter && item.semester === semesterFilter) score += 3;
